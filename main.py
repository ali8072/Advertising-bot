...
SESSION = os.environ.get("TG_SESSION") or "selfbot"
...
def load_data() -> Dict[str, Any]:
    ...
    for idx, path in enumerate([DATA_FILE] + backup_paths()):
        try:
...
def protect_session_file() -> None:
    path = os.path.join(BASE_DIR, SESSION + ".session")
...
def register(app: App) -> None:
    ...
    async def on_out(event: Any) -> None:
        ...
        if low == "!export":
            ...
            await client.send_file(chat_id, DATA_FILE, caption="data.json")
            log.info("export")
            return
        if low == "!import":
            if app.current_op():
    ...
    async def on_cb(event: Any) -> None:
        ...
        if data == "m:ex":
            await ack()
            await app.flush(force=True)
            await client.send_file(event.chat_id, DATA_FILE, caption="data.json")
...
async def main() -> None:
    ...
    if not api_id or not api_hash:
        raise SystemExit("TG_API_ID و TG_API_HASH را در محیط یا بالای فایل بگذار.")
    ...
    log.info("start as id=%s", app.me.id)
    print("Ready. در Saved Messages بفرست: !menu")
